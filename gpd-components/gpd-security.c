...
extern EmberStatus emberSerialGuaranteedPrintf(uint8_t,PGM_P formatString,...);
#if defined(EMBER_AF_PLUGIN_EMBER_MINIMAL_PRINTF)
#define gpdDebugPrintf(...) emberSerialGuaranteedPrintf(0x21, __VA_ARGS__)
#else
                                ...
                                uint8_t authLength)
{
  ...
  memcpy(&(data[authStart + authLength]), tempDataOut, payloadLength);
  if (status == 0) {
    return SUCCESS;
  }
  return FAILED;
                                  ...
                                  uint8_t* mic)
{
  ...
  memcpy(data, (uint8_t *)(&(addr->id.srcId)), 4);
...
#if defined(EMBER_AF_PLUGIN_APPS_APPLICATION_ID) && (EMBER_AF_PLUGIN_APPS_APPLICATION_ID == EMBER_GPD_APP_ID_SRC_ID)
  memcpy(&inData[0], &(addr->id.srcId), 4);
#elif defined(EMBER_AF_PLUGIN_APPS_APPLICATION_ID) && (EMBER_AF_PLUGIN_APPS_APPLICATION_ID == EMBER_GPD_APP_ID_IEEE_ID)
  memcpy(inData, &(addr->id.ieee), 4);
#else
#error "Application Id Not supported"
#endif

  memcpy(&inData[4], pKeyEncrypted, 16);
  memcpy(&inData[20], pKeyMic, 4);
                            ...
                            uint8_t authLength)
{
  ...
  memcpy(&rxMpdu[authStart + authLength], tempDataOut, encrLength);
                          ...
                          uint32_t frameCounter)
{
  ...
  memcpy(&nonceTx[0], addr->id.ieee, 8);
#else
...
#elif defined(EMBER_AF_PLUGIN_APPS_APPLICATION_ID) && (EMBER_AF_PLUGIN_APPS_APPLICATION_ID == EMBER_GPD_APP_ID_IEEE_ID)
  //TODO : check LSB first
  memcpy(&nonceRx[0], addr->id.ieee, 8);
